...
function approve_comment($comment_id){
        global $db;
		$sql = "UPDATE blog_comments SET status = 'approved' WHERE  `id` = '$comment_id' LIMIT 1";
        $result = $db->query($sql) or error ('Unable to update comment', __FILE__, __LINE__, $db->error());
	return true;
    }
    ...
    function ban_ip($comment_id){
        ...
        $ip = preg_replace('/[^\d\.]/', '', $c['ip_address']);
        $time = time();
        
        $query = "INSERT INTO {$db->prefix}bans (ip, time) VALUES (\"{$ip}\", {$time}) ON DUPLICATE KEY UPDATE ip = ip";
        ...
        <div class=\"col-xs-".$total." " . ($offset!='0'?'col-xs-offset-'.$offset:'') . " \">
            <div class=\"panel panel-default panel-comment\">
                ...
                <div class=\"panel-body\">
                    ...
                    <br/><br/><br/>
                    <a href=\"comments?approve=1&id=" . $c['id'] . "\" class=\"btn btn-primary btn-green\" >Approve</a>
                    <a href=\"javascript:reply('" . $c['id'] . "','" . $c['release_id'] . "','comments?approve=1&id=" . $c['id'] . "')\" class=\"btn btn-primary btn-blue\" >Reply & Approve</a>
                    <a href=\"comments?delete=1&id=" . $c['id'] . "\" class=\"btn btn-primary btn-red\" >Delete</a>
                    <a href=\"comments?ban=1&id=" . $c['id'] . "\" class=\"btn btn-primary btn-red\" >Ban IP</a>
                </div>
            </div>
        </div>";
        ...
        <div class=\"col-xs-".$total." " . ($offset!='0'?'col-xs-offset-'.$offset:'') . " \">
            <div class=\"panel panel-default panel-comment\">
                ...
                <div class=\"panel-body\">
                    " . $c['text'] . "
                    <br/><br/><br/>
                    <a href=\"comments?pending=1&id=" . $c['id'] . "\" class=\"btn btn-primary btn-green\" >Set Pending</a>
                    <a href=\"javascript:reply('" . $c['id'] . "','" . $c['release_id'] . "','comments?approve=1&id=" . $c['id'] . "')\" class=\"btn btn-primary btn-blue\" >Reply</a>
                    <a href=\"comments?delete=1&id=" . $c['id'] . "\" class=\"btn btn-primary btn-red\" >Delete</a>
                    <a href=\"comments?ban=1&id=" . $c['id'] . "\" class=\"btn btn-primary btn-red\" >Ban IP</a>
                </div>
            </div>
