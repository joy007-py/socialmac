...
function get_release($release_id) {
    global $db;
    $result = $db->query("SELECT rewrite_id, release_id, title, original, summary, publish_date, active, price, company_text, company_url, product_text, product_url, download_text, download_url, image_url FROM rewrites WHERE release_id='" . intval($release_id) . "'");
    $result = $db->fetch_all_assoc($result);
    
    // echo $_SESSION["image1"];
   // echo intval($release_id);
    
    $sql1 = "SELECT rewrite_id, release_id, title, original, summary, publish_date, active, price, company_text, company_url, product_text, product_url, download_text, download_url, image_url FROM rewrites WHERE release_id=intval($release_id)";
   // $result2 = mysql_query($sql1);
//  while($row = mysql_fetch_array($result))
  {
  //echo $row['CustName'] . " " . $row['OrderNumber'];
  //echo "<br />";
  //echo $row['DateTimeIn'] . " " . $row['Contact'];
...
function release_rewrite($release) {
    ...
    if ($release['product_text'] != "" && $release['product_url'] != "" && $release['price'] != "") {
        ...
        if ($release['download_url'] != "" && $release['download_text'] != "") {
            ...
            /*
              if(strpos($release['download_url'], 'itunes.apple.com')){
              echo ' the <a href="' . $release['download_url'] . '" target="_blank">App Store</a>';
              }else{
              echo ' their <a href="' . $release['download_url'] . '" target="_blank">website</a>';
              }
             */
        }
...
function get_votes($release_id) {
    ...
    // echo $_SESSION["image1"];
    $sql1 = "SELECT rewrite_id, release_id, title, original, summary, publish_date, active, price, company_text, company_url, product_text, product_url, download_text, download_url, image_url FROM rewrites WHERE release_id= $release_id";
	$data = $data[key($data)];
	$results['image_url'] = $data['image_url'];
	echo $results['image_url'];
  //  echo intval($release_id);   
...
/*	session_start();
	//echo $_SESSION["image1"];
	echo $releaseInfo['image_url'];
	$imageData = base64_encode(file_get_contents($_SESSION["image1"]));
